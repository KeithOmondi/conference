import React, { useState, useEffect, forwardRef, type ReactNode, type CSSProperties } from "react";
...
>
{pageContent} <div className="absolute bottom-2 right-2 text-xs text-gray-600 bg-gray-100 rounded-tl-lg px-2 py-1 opacity-80">
(Simulated Flip Book) </div> </div>
);
}
);
...
className={`w-full h-full flex flex-col justify-between items-center shadow-inner p-6 relative
        ...
        ${pageType === "cover" ? "bg-[#005A2B] text-white" : "bg-white border-l border-gray-300"}
        ${isFlipping ? "scale-105 opacity-0" : "scale-100 opacity-100"}`}
style={{ minWidth: "360px", minHeight: "520px" }}
> <div className="flex flex-col w-full h-full justify-center items-center">
{children} </div>
{number > 1 && ( <div className="absolute bottom-4 right-6 text-sm text-gray-500">
{number - 1} </div>
)} </div>
)
);
...
// --------------------------------------------------------------
if (loading) {
return ( <div className="p-4 text-center text-[#005A2B] font-semibold">
...
// --------------------------------------------------------------
const messagesPages = [
{ pageType: "cover" as const, type: "cover", content: {} },
...presentations.map((p) => ({
pageType: "content" as const,
type: "messages" as const,
content: [
{ sender: `${p.presenter.firstName} ${p.presenter.lastName}`, content: p.title },
{ sender: "Description", content: p.description || "No description provided." },
{ sender: "Download File", content: p.fileUrl || "No file uploaded." },
],
})),
{ pageType: "cover" as const, type: "cover", content: { image } },
];
...
return ( <div className="w-full h-full flex flex-col justify-center items-center p-8 text-white text-center"> <h1 className="text-3xl font-extrabold border-b-2 border-[#C6A64F] pb-4">
ANNUAL HIGH COURT HUMAN RIGHTS SUMMIT </h1> </div>
);
}
    ...
    <>
      ...
      <ul className="space-y-3 w-full overflow-y-auto">
        {pageData.content.map((msg: any, i: number) => (
          <li key={i} className="bg-gray-100 p-3 rounded-lg shadow-sm text-sm border">
            <span className="font-semibold text-[#005A2B]">{msg.sender}:</span>{" "}
            {msg.content}
          </li>
        ))}
      </ul>
...
// Render Flip Book
// --------------------------------------------------------------
return ( <div className="flex flex-col items-center p-4"> <HTMLFlipBook className="shadow-2xl rounded-xl" width={360} height={520} currentPage={currentPage}>
{messagesPages.map((pageData, index) => (
<Page key={index} number={index + 1} isFlipping={isFlipping} pageType={pageData.pageType}>
{renderPageContent(pageData, index)} </Page>
))} </HTMLFlipBook>
