...
// Loader component
const LoginLoader: React.FC<{ imageUrl: string }> = ({ imageUrl }) => (

  <div className="fixed inset-0 flex flex-col items-center justify-center bg-[#005A2B]/90 backdrop-blur-md text-white z-50 transition-opacity duration-300">
    <img
      src={imageUrl}
      alt="Justice Logo"
      className="w-36 h-36 mb-8 animate-spin-slow border-4 border-[#C6A64F] rounded-full p-2"
    />
    <h1 className="text-4xl md:text-5xl font-extrabold text-center px-6 leading-tight">
      WELCOME TO ANNUAL HIGH COURT HUMAN RIGHTS SUMMIT 2025
    </h1>
    <p className="mt-4 text-xl italic text-[#C6A64F]">
...
// Start 10-second loader timer after login success
useEffect(() => {
if (user && showLoader) {
...
useEffect(() => {
if (user && isAnimationComplete) {
navigate("/", { replace: true });
}
}, [user, isAnimationComplete, navigate]);
...
if (loading) return;
  ...
  setShowLoader(true);
  const res = await dispatch(loginUser({ pj: pjNumber }));

  if (!loginUser.fulfilled.match(res)) {
    // If login failed, hide loader immediately
    setShowLoader(false);
  }
},
...
// Show loader if login is in progress or animation is running
if (showLoader) return <LoginLoader imageUrl={JUDGE_IMAGE_URL} />;

return ( <div className="min-h-screen flex items-center justify-center bg-gray-100 p-6 font-sans"> <div className="bg-white shadow-2xl rounded-2xl w-full max-w-xl overflow-hidden border border-gray-100">
{/* --- Header Section --- */} <div className="relative p-10 pt-16 bg-white overflow-hidden"> <div className="absolute top-0 left-0 w-full h-full bg-[#005A2B] opacity-3" /> <div className="absolute -top-10 -right-10 w-48 h-48 bg-[#C6A64F] opacity-10 rounded-full blur-xl" /> <div className="absolute top-0 right-0 w-1/3 h-1/2 bg-[#005A2B] opacity-5 transform skew-y-6" />
      ...
      <div className="relative z-10 text-center">
        ...
        <p className="mt-3 text-base text-gray-700 leading-relaxed text-justify">
          It is my personal pleasure to welcome you to the <strong>2025 Annual
          High Court Human Rights Summit</strong>. As always, we remain committed
          to strengthening access to justice, promoting fairness, and
          upholding constitutional guarantees for all.
        </p>

        <p className="mt-6 font-extrabold text-[#005A2B] text-lg">
          Hon. Justice Eric Ogola, EBS
        </p>
        <p className="text-base text-gray-600 -mt-1">Principal Judge, High Court</p>
      </div>
    </div>
